...
{	
	...
	$avg_star = DB::table('reviews')->where('product_id',$id)->avg('rating');
	//dd($avg_star);
	$rate = round($avg_star);
	//dd($rate);
              
		 $shop = DB::table('products')->get()->take(9);

		 return view('shop.shop-detail',['product_detail'=>$product_detail, 'shop'=>$shop,'rate'=>$rate]); 
